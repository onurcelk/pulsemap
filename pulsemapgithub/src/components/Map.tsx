import { MapContainer, TileLayer, Marker, Popup, useMap, ZoomControl, LayersControl, Polyline } from 'react-leaflet';
...
const createStrategicIcon = (type: string) => {
  ...
  const iconHtml = (STRATEGIC_ICON_SVG[type] || STRATEGIC_ICON_SVG.military).replace(/\{\{S\}\}/g, String(iconSize));
...
const createAircraftIcon = (aircraft: LiveAircraft) => {
  ...
  const fillColor = isGrounded ? "rgba(100, 116, 139, 0.6)" : "#22c55e"; // slate-500 vs green-500
  const strokeColor = isGrounded ? "rgba(255, 255, 255, 0.4)" : "white";
  const dropShadow = isGrounded ? "none" : "drop-shadow(0px 2px 4px rgba(0,0,0,0.8))";
...
const RadarHighlight = ({ lat, lng, children, isLiveMode }: { lat: number, lng: number, children: React.ReactNode, isLiveMode: boolean }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      let markerAngle = (Math.atan2(point.y - centerPoint.y, point.x - centerPoint.x) * 180 / Math.PI) + 90;
  ...
  return (
    <div className={isLit ? 'radar-glow-active' : ''}>
      {children}
    </div>
  );
};
...
export default function Map({
  events, center, zoom, onEventClick, selectedEventId,
  isDarkMode = false, isLiveMode = false,
  strategicAssets = [], showStrategicAssets = false,
  liveAircraft = [], liveShips = []
}: MapProps) {
  return (
        ...
        {isLiveMode && (
          ...
          >
            ...
            <div className="absolute top-4 right-20 flex items-center gap-2 px-3 py-1 bg-black/80 rounded-full border border-red-500/50">
              ...
              <span className="text-[10px] font-mono text-red-500 font-black uppercase tracking-widest">Scanning Signal...</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      >
        ...
        <MarkerClusterGroup
          ...
          iconCreateFunction={(cluster: any) => {
            ...
            const dominant = Object.entries(catCounts).sort((a, b) => b[1] - a[1])[0]?.[0] as EventCategory || 'other';
                ...
                <div className="p-3 max-w-[280px] bg-[var(--bg)]">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {(event as any).hotScore > 1 && (
                      <span className="text-[9px] font-mono font-bold text-orange-400 ml-auto">🔥 {(event as any).hotScore} sources</span>
                    )}
                    {!((event as any).hotScore > 1) && (
                      <span className="text-[9px] font-mono opacity-40 ml-auto">{new Date(event.timestamp).toLocaleTimeString()}</span>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-1.5 max-h-[220px] overflow-y-auto pr-0.5">
                    {((event as any).relatedStories || [{ title: event.title, sourceUrl: event.sourceUrl, source: '', timestamp: event.timestamp }]).map((s: any, i: number) => (
                      <a
                        key={i}
                        href={s.sourceUrl || '#'}
                        target="_blank"
                      ...
                      >
                        <div className={`p-1.5 rounded border transition-all ${i === 0 ? 'border-[var(--accent)]/30 bg-[var(--accent)]/5' : 'border-[var(--line)]/50 hover:border-[var(--line)]'}`}>
                          <p className={`text-[11px] font-semibold leading-tight ${i === 0 ? 'text-[var(--ink)]' : 'text-[var(--ink-dim)]'} group-hover:text-[var(--ink)] line-clamp-2`}>
                            {s.title}
                          </p>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            {s.source && <span className="text-[8px] font-mono opacity-40">{s.source}</span>}
                            <span className="text-[8px] font-mono opacity-30 ml-auto">{new Date(s.timestamp).toLocaleTimeString()}</span>
                          </div>
                        </div>
                      </a>
                    ))}
        ...
        {showStrategicAssets && strategicAssets.map((asset) => (
          <Marker
            key={asset.id}
            ...
            <Popup className="strategic-popup">
              <div className="p-3 max-w-[200px] text-[var(--ink)]">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[10px] font-black uppercase text-indigo-400 tracking-widest">{asset.type} Asset</span>
                </div>
                <h4 className="font-bold text-sm uppercase mb-1 text-[var(--ink)]">{asset.name}</h4>
                <p className="text-[10px] text-[var(--ink-dim)] leading-relaxed mb-3">{asset.description}</p>
                {asset.wikiUrl && (
                  <a
                    href={asset.wikiUrl}
                    target="_blank"
        ...
        {isLiveMode && liveAircraft.map((ac) => (
          <React.Fragment key={ac.id}>
            {ac.history && ac.history.length > 1 && (
              <Polyline
                positions={ac.history.map(pos => [pos[0], pos[1]])}
                color={ac.alt <= 100 && ac.speed < 20 ? "rgba(100, 116, 139, 0.4)" : "rgba(34, 197, 94, 0.6)"}
                weight={2}
                dashArray="4 4"
              />
            )}
            ...
            >
              ...
              <Popup className="strategic-popup">
                <div className="p-3 max-w-[200px] bg-slate-900 text-white rounded-lg border border-green-500/30">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[10px] font-black uppercase text-green-400 tracking-widest flex items-center gap-1">
                  ...
                  {(ac.operator || ac.registration) && (
                    <div className="text-[10px] font-black uppercase text-green-500/80 mb-3 truncate bg-green-900/40 px-2 py-1.5 rounded-md inline-block border border-green-500/20">
                      {ac.operator || `Reg: ${ac.registration}`}
                    </div>
                  )}
                  <div className="grid grid-cols-2 gap-2 text-[10px] font-mono">
                    <div className="bg-black/40 p-1.5 rounded border border-white/10">
                      <div className="opacity-50 text-[8px] uppercase tracking-wider mb-0.5">Altitude</div>
                      <div className="font-bold text-green-400">{Math.round(ac.alt)} ft</div>
                    </div>
                    <div className="bg-black/40 p-1.5 rounded border border-white/10">
                      <div className="opacity-50 text-[8px] uppercase tracking-wider mb-0.5">Speed</div>
                      <div className="font-bold text-green-400">{Math.round(ac.speed)} kts</div>
                    </div>
                    <div className="bg-black/40 p-1.5 rounded border border-white/10">
                      <div className="opacity-50 text-[8px] uppercase tracking-wider mb-0.5">Heading</div>
                      <div className="font-bold text-green-400">{Math.round(ac.heading)}°</div>
                    </div>
                    <div className="bg-black/40 p-1.5 rounded border border-white/10">
                      <div className="opacity-50 text-[8px] uppercase tracking-wider mb-0.5">Type</div>
                      <div className="font-bold text-white truncate">{ac.type}</div>
                    </div>
                  </div>
                  <div className="mt-2 text-[9px] opacity-60 leading-tight">
                    {ac.description}
                  </div>
                </div>
              </Popup>
              ...
              <div className="p-3 max-w-[200px] bg-slate-900 text-white rounded-lg border border-sky-500/30">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[10px] font-black uppercase text-sky-400 tracking-widest flex items-center gap-1">
                    <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="animate-pulse"><path d="M12 2 L9 8 L9 20 L12 22 L15 20 L15 8 Z" /><path d="M10 12 L12 10 L14 12 L14 16 L10 16 Z" /></svg> OSINT Tracker
                  </span>
                </div>
