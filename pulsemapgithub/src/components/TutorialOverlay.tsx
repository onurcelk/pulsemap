...
import { Map, Layers, Flame, ShieldAlert, X, ChevronRight, CheckCircle2 } from 'lucide-react';
...
export default function TutorialOverlay({ onComplete, isOpen }: TutorialOverlayProps) {
    ...
    if (targetRect) {
        ...
        if (isMobile) {
            ...
            if (isTargetAtBottom) {
                modalStyle = { top: padding, left: padding, right: padding, transform: 'none' };
            } else {
                modalStyle = { bottom: padding, left: padding, right: padding, transform: 'none' };
            }
        } else {
            ...
            >
                <AnimatePresence mode="wait">
                    <motion.div
                        ...
                        style={{
                            borderColor: `var(--${slide.bg.split('-')[1]}-500)`,
                        }}
                    >
                        {/* Header */}
                        <div className="flex items-center justify-between p-4 border-b border-[var(--line)]/50 bg-[var(--line)]/10">
                            <div className="flex items-center gap-3">
                                ...
                                <h2 className="text-sm font-black uppercase tracking-widest text-[var(--ink)]">{slide.title}</h2>
                            </div>
                        ...
                        <div className="bg-[var(--line)]/10 p-4 flex items-center justify-between">
                            ...
                            <button
                                onClick={nextSlide}
                                className="flex items-center gap-2 bg-[var(--ink)] text-[var(--bg)] px-5 py-2 rounded-lg font-bold uppercase tracking-widest text-[10px] hover:bg-[var(--accent)] hover:text-white transition-colors"
                            >
                                {currentSlide === SLIDES.length - 1 ? (
                                    <>Enter System <CheckCircle2 size={14} /></>
                                ) : (
