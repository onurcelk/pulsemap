...
const textTitle = 'Paris (AFP) - France strongly condemns the recent drone strikes hitting a rebel base in Iran.';
const textDesc = 'The French president issued a statement from Paris today regarding the attacks in Iran.';

const titleDoc = nlp(textTitle);
const titlePlaces = titleDoc.places().out('array').map(p => p.toLowerCase().trim());
const descDoc = nlp(textDesc);
const descPlaces = descDoc.places().out('array').map(p => p.toLowerCase().trim());

const PUBLISHER_HUBS = ["paris", "france", "london", "uk", "united kingdom", "washington dc", "new york", "usa", "united states", "geneva", "brussels", "moscow", "russia", "beijing", "china", "dubai", "doha"];
